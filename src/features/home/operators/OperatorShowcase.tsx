      ...
      <Container>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-4">
          {VERIFIED_OPERATORS.map((op, idx) => (
            <Card
              key={idx}
              className="group rounded-2xl border border-border/80 bg-white p-4 shadow-subtle hover:border-slate-300 hover:shadow-hover transition-all duration-normal"
            >
              <div className="flex flex-col justify-between h-full space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-700 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                    <Building2 className="h-4 w-4" />
                  </div>
                  <span className="inline-flex items-center rounded-md bg-amber-50 px-1.5 py-0.5 text-[11px] font-semibold text-amber-700">
