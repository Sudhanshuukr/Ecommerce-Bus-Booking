...
export function HowItWorksSection({ className }: HowItWorksSectionProps) {
  return (
    <Section spacing="lg" className={cn('bg-slate-50/50 py-12 md:py-16', className)}>
      <Container>
        {/* Section Header */}
        <div className="mx-auto mb-12 flex max-w-2xl flex-col items-center text-center space-y-2">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {STEPS.map((item, idx) => {
            ...
            return (
              <Card
                key={idx}
                className="relative flex flex-col justify-between rounded-2xl border border-border/80 bg-white p-6 shadow-subtle hover:border-slate-300 transition-all duration-normal"
              >
