...
import { MapPin, Calendar, Users, SlidersHorizontal, ChevronDown } from 'lucide-react';
...
export const SearchResultsHeader = React.memo<SearchResultsHeaderProps>(function SearchResultsHeader({
  ...
  return (
    <div
      className={cn(
        'flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-border/80 bg-white p-4 sm:p-5 shadow-subtle',
        className
      )}
    >
      ...
      {/* Edit Search Toggle Button */}
      <div className="flex items-center space-x-3">
        <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full">
          {resultCount} {resultCount === 1 ? 'Bus Available' : 'Buses Available'}
        </span>
