...
export function OperatorSidebar({ className }: { className?: string }) {
  ...
  const navItems = [
    ...
    { label: 'Bookings', href: '/operator/bookings', icon: Ticket },
  ];

  return (
    <aside
      aria-label="Operator Console Sidebar"
      className={cn(
    ...
    >
      ...
      <div className="space-y-3 px-2 pt-2 border-b border-slate-800 pb-4">
        <Logo size="default" />
        <div className="flex items-center space-x-2 text-[11px] font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-800/60 rounded-lg px-2.5 py-1 w-fit">
      ...
      <div className="pt-4 border-t border-slate-800 px-2 text-[11px] text-slate-500">
        <p className="font-semibold text-slate-400">Operator Console v1.0</p>
        <p className="mt-0.5">Fleet & Schedule Management Portal.</p>
      </div>
    </aside>
  );
}
